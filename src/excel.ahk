...
Monghoaluc(modehl := "daily") {
    ...
    loop
    {
      ...
      if (ok:=FindText(MongHoaX := "wait", MongHoaY := 5, 18-150, 104-150, 18+150, 104+150, 0, 0, MongHoa))
      {
        ...
        Sleep, 1000
        ; TODO
        MouseClick, left, 87, 688
        Sleep, 1000
        MouseClick, left, 720, 300
        Sleep, 1000
        FormatTime, CurrentDate, , dd/MM/yyyy
        Send, {Text}Now:%CurrentDate%
        ; TODO
        MouseClick, left, 847, 542
        break
      }
    }
...
CountOk() {
    ...
    if (ok:=FindText(X := "wait", Y := 0.5, 401-150, 521-150, 401+150, 521+150, 0, 0, TextOk1))
    {
        ...
        {
            seen[key] := true
            positions.Push({x: X, y: Y, pattern: 1})
            count++
        }
    }
    ...
    if (ok:=FindText(X := "wait", Y := 0.5, 530-150, 523-150, 530+150, 523+150, 0, 0, TextOk2))
    {
        ...
        {
            seen[key] := true
            positions.Push({x: X, y: Y, pattern: 2})
            count++
        }
    }
    ...
    if (ok:=FindText(X := "wait", Y := 0.5, 657-150, 521-150, 657+150, 521+150, 0, 0, TextOk3))
    {
        ...
        {
            seen[key] := true
            positions.Push({x: X, y: Y, pattern: 3})
            count++
        }
    }
    ...
    if (ok:=FindText(X := "wait", Y := 0.5, 785-150, 523-150, 785+150, 523+150, 0, 0, TextOk4))
    {
        ...
        {
            seen[key] := true
            positions.Push({x: X, y: Y, pattern: 4})
            count++
        }
    }
  ...
  if (ok:=FindText(X := "wait", Y := 0.5, 915-150, 523-150, 915+150, 523+150, 0, 0, TextOk5))
    {
        ...
        {
            seen[key] := true
            positions.Push({x: X, y: Y, pattern: 5})
            count++
        }
    }
...
LichSuMuaban() {
    ShowFunctionTooltip("LichSuMuaban")
    Send, {f5}
    Sleep, 2500
    MouseClick, left, 818, 735
    Hoat:="|<>*91$10.7US1s7US000001s7US1s7W"
...
ChuanBiLikeTrangVien() {
    Send, {Enter}
    Sleep, 500
    MouseClick, left, 15, 60
    Sleep, 500
    MouseClick, left, 90, 695
    Sleep, 500
    ; input
    MouseClick, left, 195, 740
    Sleep, 500
    ; Tieu Man nhi
    Send, {Text}2056203502
    ; search
    MouseClick, left, 300, 735
    Sleep, 500
    ; them chat
    MouseClick, left, 58, 111
    Sleep, 500
    MouseClick, left, 117,221
    Sleep, 500
    Send, {Esc}
    Sleep, 500
    Send, {Esc}
